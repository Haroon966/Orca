...
export const api = {
  ...
  // Claude config management (ClaudeUI)
  claudeConfig: {
    claudeMd: (projectPath) => {
      const query = projectPath ? `?projectPath=${encodeURIComponent(projectPath)}` : '';
      return authenticatedFetch(`/api/claude-config/claude-md${query}`);
    },
    hooks: (projectPath) => {
      const query = projectPath ? `?projectPath=${encodeURIComponent(projectPath)}` : '';
      return authenticatedFetch(`/api/claude-config/hooks${query}`);
    },
    memory: () => authenticatedFetch('/api/claude-config/memory'),
  },
